 ...
 * Solving these two ODEs using trapezoidal approximation
 *
 * Notes:
 ...
 * The only difference between BE and Trapezoidal is the A matrix
 * Using basic pointer notation for matrix manipulation here
 */

// A function to invert a 2x2 matrix
...
// A function to multiply a 2x2 matrix with a 2x1 vector
...
int main (void) {
   ...
   double       v1_n    = 0.0;         // v1(t). C1 is initially discharged
   double       v2_n    = 0.0;         // v2(t). C2 is initially discharged
   double       v1_nn   = 0.0;         // v1(t+h)
   double       v2_nn   = 0.0;         // v2(t+h)
   double       vs_n    = 0.0;         // The voltage source, vs(t)
   double       vs_nn   = 0.0;         // The voltage source, vs(t+h)
   double       g1      = 1.0/r1;
   double       g2      = 1.0/r2;
   ...
   do {
      ...
      // trapezoidal
      A [0][0] = 1.0 + ((h/(2*c1))*(g1+g2));  // argument for v1_nn (node 1)
      A [0][1] = -(h/(2*c1))*g2;              // argument for v2_nn (node 1)
      A [1][0] = -(h/(2*c2))*g2;              // argument for v1_nn (node 2)
      A [1][1] = 1.0 + ((h/(2*c2))*g2);       // argument for v2_nn (node 2)
      ...
      b [0] = v1_n;
      b [0] = b [0] + ((h*g1)/(2*c1))*(vs_nn+vs_n-v1_n);
      b [0] = b [0] + ((h*g2)/(2*c1))*(v2_n-v1_n);

      // Output of equation 2
      b [1] = v2_n;
      b [1] = b [1] + ((h*g2)/(2*c2))*(v1_n-v2_n);
